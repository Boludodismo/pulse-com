import {isAutomaticQuoteTag} from "@shared/quoteHistory";
...
export function CareHistory({clientId}:{clientId?:number}) {
 ...
 const {user}=useAuth();const manager=user?.role==='admin'||user?.role==='superadmin';
 const link=trpc.customerCare.linkFeedback.useMutation({onSuccess:()=>utils.customerCare.history.invalidate(),onError:e=>toast.error(e.message)});
...
export function ClientCare({clientId}:{clientId:number}) {
 const utils=trpc.useUtils();const tags=trpc.customerCare.tags.useQuery({clientId},{refetchInterval:30000});const [draft,setDraft]=useState('');
 ...
 return <section className="space-y-4 rounded-xl border p-4 my-4"><h2 className="font-semibold">Etiquetas do cliente</h2><p className="text-sm text-muted-foreground">Exemplos: realismo, fine line, cover-up, retorno. Pesquise a etiqueta na lista de clientes.</p><div className="flex gap-2 flex-wrap">{tags.data?.map(t=>isAutomaticQuoteTag(t.label)?<Badge key={t.label} variant="secondary" title="Etiqueta automática do histórico de orçamentos">{t.label} · automática</Badge>:<Button size="sm" variant="outline" key={t.label} onClick={()=>save.mutate({clientId,labels:tags.data!.filter(x=>x.label!==t.label).map(x=>x.label)})} aria-label={`Remover etiqueta ${t.label}`}>{t.label} ×</Button>)}</div><form className="flex gap-2" onSubmit={e=>{e.preventDefault();if(isAutomaticQuoteTag(draft)){toast.info('Esta etiqueta é aplicada automaticamente pelo histórico de orçamentos.');return;}if(draft.trim())save.mutate({clientId,labels:[...(tags.data||[]).map(t=>t.label),draft.trim()]});}}><Input aria-label="Nova etiqueta" maxLength={60} value={draft} onChange={e=>setDraft(e.target.value)} placeholder="Nova etiqueta"/><Button disabled={save.isPending||tags.isLoading||!!tags.error}>Adicionar etiqueta</Button></form><CareHistory clientId={clientId}/></section>;
}
