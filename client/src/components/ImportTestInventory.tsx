...
export default function ImportTestInventory({artists,onComplete}:{artists:{id:number;name:string}[];onComplete:()=>void}) {
 ...
 const run=async()=>{
   ...
   try{
     while(offset<TECHNICAL_CATALOG_2026.length){
       const result=await mutation.mutateAsync({artistId:Number(artist),offset});
 ...
 return <details className="rounded-lg border border-orange-500/30 p-4">
   ...
   <div className="mt-3 space-y-3 text-sm">
     ...
     <Button disabled={!artist||running} onClick={run}>{running?`Importando ${progress}/${TECHNICAL_CATALOG_2026.length}…`:'Adicionar estoque fictício e estimativas'}</Button>
