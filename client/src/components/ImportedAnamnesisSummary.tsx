    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          ...
          {forms.every(form=>form.sessionDate)
            ? ' As datas das sessões foram confirmadas pelo responsável, usando o dia do envio de cada formulário.'
            : ' A data de realização da sessão precisa ser confirmada nas fichas ainda pendentes.'}
        ...
        {forms.map((form, index) => (
          <article key={`${form.key}-${index}`} className="rounded-lg border p-4 space-y-3">
            ...
            <dl className="grid gap-4 sm:grid-cols-2">
              ...
              <div><dt className="text-sm text-muted-foreground">Data da sessão / procedimento</dt><dd>{form.sessionDate||'A confirmar'}</dd></div>
