...
export default function MaterialSpecificationFields({value,onChange,materials}:{value:SpecificationForm;onChange:(key:keyof SpecificationForm,value:string)=>void;materials:any[]}){
 ...
 const fields:[keyof SpecificationForm,string][]=[['category','Categoria'],['brand','Marca'],['line','Linha / variante'],['model','Modelo / SKU'],...(needle?[['needleCount','Quantidade de pontas'],['configuration','Formato da ponta'],['gauge','Calibre — código do fabricante'],['diameter','Diâmetro (mm)'],['taper','Taper']] as [keyof SpecificationForm,string][]:[]),...(!needle?[['configuration',cup?'Tamanho do batoque':'Tamanho / configuração']] as [keyof SpecificationForm,string][]:[]),['packageQuantity','Unidades por embalagem'],['purchaseUnit','Tipo de embalagem (caixa, frasco…)']];
 return <div className="space-y-3"><p className="text-sm text-muted-foreground">Busque uma opção existente ou digite uma nova categoria, marca ou variante. Ela ficará disponível nos próximos cadastros do seu estoque.</p><div className="grid gap-3 sm:grid-cols-2">{fields.map(([key,label])=><div key={key}><Label htmlFor={'spec-'+key}>{label}</Label><select aria-label={'Selecionar '+label} className="w-full rounded border bg-background p-2 text-sm mb-1" value={options(key).includes(value[key])?value[key]:''} onChange={e=>onChange(key,e.target.value)}><option value="">Selecionar existente ou digitar abaixo</option>{options(key).map(v=><option key={v} value={v}>{v}</option>)}</select><Input id={'spec-'+key} list={'options-'+key} value={value[key]} onChange={e=>onChange(key,e.target.value)} type={key==='needleCount'||key==='packageQuantity'?'number':'text'} min={1} step={1}/><datalist id={'options-'+key}>{options(key).map(v=><option key={v} value={v}/>)}</datalist></div>)}</div>{cup&&<p className="text-xs text-muted-foreground">Cadastre cada tamanho de batoque separadamente (PP, P, M, G, GG ou medida do fabricante), com unidade de estoque “un”. A embalagem pode conter vários batoques; o uso na sessão é contado individualmente.</p>}{needle&&<p className="text-xs text-muted-foreground">Código de calibre e diâmetro são campos separados. Confira os valores na embalagem do fabricante.</p>}</div>;
}
