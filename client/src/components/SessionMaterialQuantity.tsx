...
type Props = { material: QuantityMaterial; value: string; onChange: (quantity: string) => void; disabled?: boolean; materials?: QuantityMaterial[]; onMaterialChange?: (id: string) => void };
export default function SessionMaterialQuantity({material,value,onChange,disabled,materials=[],onMaterialChange}: Props) {
  const [mode,setMode] = useState<'drops'|'cup'>('drops');
  ...
  const update = (n: string, m=mode, s=size) => { setCount(n); onChange(inkStockQuantity(material.unit, Number(n), m, s==='custom'?Number(customMl):s)); };
  return <div className="space-y-2 min-w-0">
    ...
    {ink && <>
      <label className="block text-xs">Medida da tinta<select className={control} disabled={disabled} value={mode} onChange={e=>{const next=e.target.value as typeof mode;setMode(next);update('1',next);}}><option value="drops">Gotas</option><option value="cup">Batoque cheio</option></select></label>
    ...
    <label className="block text-xs">{ink ? mode==='drops'?'Quantidade de gotas':'Quantidade de batoques cheios' : cup?'Quantidade de unidades':`Quantidade (${material.unit})`}
      <input className={control} aria-label={`Quantidade de ${material.name}`} type="number" min={ink||cup?'1':'0.001'} step={ink||cup?'1':'0.001'} inputMode={ink||cup?'numeric':'decimal'} disabled={disabled} value={ink?count:value} onChange={e=>ink?update(e.target.value):onChange(cup && !Number.isInteger(Number(e.target.value))?'':e.target.value)}/>
    </label>
    {ink && <p className="text-xs text-muted-foreground">{value ? `Baixa de tinta: ${Number(value).toLocaleString('pt-BR')} ${material.unit}.` : 'Informe uma quantidade inteira e cadastre a tinta em ml ou gotas.'} Conversão da sessão: 20 gotas/ml. O batoque vazio é registrado separadamente.</p>}
  </div>;
}
