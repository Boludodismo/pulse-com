...
import { inkStockQuantity, isSessionCup, isSessionInk, isSessionDiluent, sessionCupSize, SESSION_DROPS_PER_ML, sessionCupSizeLabel, sessionMaterialName, SESSION_CUP_ML, type SessionCupSize, type QuantityMaterial } from '@shared/sessionInkQuantity';
...
export default function SessionMaterialQuantity({material,value,onChange,disabled,materials=[],onMaterialChange,planning=false}: Props) {
  ...
  const cup = isSessionCup(material), ink = isSessionInk(material), diluent = isSessionDiluent(material), actualSize = sessionCupSizeLabel(material), capacitySize=sessionCupSize(material);
  ...
  return <div className="space-y-2 min-w-0">
    {cup && <label className="block text-xs">Tamanho do batoque
      ...
      {capacitySize&&<p className="text-xs">Capacidade de referência: {SESSION_CUP_ML[capacitySize]} ml · {SESSION_CUP_ML[capacitySize]*SESSION_DROPS_PER_ML} gotas</p>}
    ...
    {ink && <p className="text-xs text-muted-foreground">{value ? `${planning ? "Quantidade prevista" : "Consumo"}: ${Number(value).toLocaleString('pt-BR')} ${material.unit}.` : 'Informe uma quantidade inteira e cadastre o material em ml ou gotas.'} Conversão da sessão: {SESSION_DROPS_PER_ML} gotas/ml. O batoque vazio é registrado separadamente.</p>}
  </div>;
}
