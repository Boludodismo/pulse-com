...
import { ManualInboxReply } from "@/components/ManualInboxReply";
...
export default function IntelligentInbox() {
  ...
  const messages = trpc.intelligentInbox.messages.useQuery({ conversationId: selectedId ?? 0, limit: 50 }, { enabled: eligible && can("inbox_conversations") && !!selectedId, refetchInterval: 15000 });
  ...
  return <main className="mx-auto w-full min-w-0 max-w-7xl space-y-5 p-3 sm:p-6">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div><h1 className="flex items-center gap-2 text-2xl font-bold"><Inbox />Central Inteligente de Atendimentos</h1><p className="mt-2 text-muted-foreground">Mensagens recebidas pelo BotConversa e respostas enviadas por você.</p></div>
      <Badge variant="outline" className={operational ? "w-fit border-emerald-600 text-emerald-500" : "w-fit"}>{operational ? "Conectada · resposta manual" : "Aguardando integração"}</Badge>
    </header>

    <Card><CardContent className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-3"><ShieldCheck className={operational ? "text-emerald-500" : "text-muted-foreground"}/><div><p className="font-semibold">{operational ? "Atendimento manual disponível" : "Integração BotConversa não encontrada"}</p><p className="text-sm text-muted-foreground">A Central envia apenas as respostas que você escrever e confirmar. Lembretes e aniversários continuam nas automações existentes.</p></div></div>{can("inbox_settings") && <Button variant="outline" onClick={() => setSection("settings")}><Settings className="mr-2 h-4 w-4"/>Ver integração</Button>}</CardContent></Card>
    ...
    {active[0] === "settings" ? <Card><CardHeader><CardTitle>BotConversa · atendimento manual</CardTitle></CardHeader><CardContent className="space-y-4"><div className="grid gap-3 sm:grid-cols-2">{[
      ...
      ["Respostas automáticas da Central", "Desativadas"], ["Resumo com IA", "Desativado"],
    ].map(([label,value]) => <div key={label} className="rounded-lg border p-3"><p className="text-sm text-muted-foreground">{label}</p><p>{value}</p></div>)}</div><p className="text-sm text-muted-foreground">Abra uma conversa para responder manualmente. O histórico mostra as mensagens encaminhadas ao CRM; ele pode não conter toda a conversa do BotConversa.</p></CardContent></Card>
    ...
    : <>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.8fr)]">
        ...
        <Card><CardHeader><CardTitle>Detalhe da conversa</CardTitle></CardHeader><CardContent>{!selected ? <p className="text-sm text-muted-foreground">Selecione uma conversa para visualizar as mensagens.</p> : <div className="space-y-4"><div><p className="font-semibold">{selected.clientName}</p><p className="text-sm text-muted-foreground">{selected.phone} · {labels[selected.classification] ?? selected.classification}</p></div><div className="max-h-[500px] space-y-2 overflow-y-auto">{messages.data?.items.slice().reverse().map(message => <div key={message.id} className="rounded-lg border bg-muted/30 p-3"><p className="mb-1 text-xs font-semibold">{message.direction === "outbound" ? "Você" : "Cliente"}</p><p className="whitespace-pre-wrap break-words text-sm">{message.textContent}</p><p className="mt-1 text-[11px] text-muted-foreground">{message.direction === "outbound" ? ({ accepted: "Aceita pelo BotConversa", failed: "Não enviada", unknown: "Envio não confirmado — confira no BotConversa", sending: "Envio em andamento ou sem confirmação — confira no BotConversa" }[message.processingStatus] ?? "Resposta manual") : "Recebida"} · {formatDate(message.messageAt)}</p></div>)}</div><ManualInboxReply key={`${user.id}:${user.studioId}:${selected.id}`} storageKey={`inbox-reply:${user.id}:${user.studioId}:${selected.id}`} conversationId={selected.id} recipient={selected.clientName || selected.phone || "Cliente"} phone={selected.phone || ""} enabled={operational && !!access.data?.permissions.find(p => p.module === "inbox_conversations")?.canWrite} /></div>}</CardContent></Card>
      </div>
    </>}
  </main>;
}
