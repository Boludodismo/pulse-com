...
function whatsappLink(phone: string | null | undefined, clientName: string, question = false) {
  ...
  const message = encodeURIComponent(
    `Olá! ${question ? "Tenho uma dúvida sobre" : "Gostaria de conversar sobre"} a proposta para ${clientName}.\n${window.location.origin}${window.location.pathname}`,
  );
...
export default function PublicProposal() {
  ...
  const question = trpc.quotes.public.question.useMutation({onSuccess:()=>{setAsking(false);setQuestionText("");toast.success("Dúvida registrada para o responsável pelo orçamento.");void query.refetch();},onError:error=>toast.error(error.message)});
  ...
  const whatsapp = useMemo(() => {
    ...
    return whatsappLink(phone, payload.client.name);
  }, [query.data]);
  const questionLink = useMemo(() => {
    const payload = query.data?.payload;
    if (!payload) return null;
    return whatsappLink(payload.editor.contactSource === "artist" ? payload.artist.phone : payload.studio.phone, payload.client.name, true);
  }, [query.data]);
  ...
  return (
    <main className="proposal-public-shell">
      ...
      <div className="proposal-public-actions">
        ...
        <button type="button" className="proposal-action-secondary" onClick={()=>setAsking(true)}><MessageCircle className="h-5 w-5"/>Tirar uma dúvida</button>
      </div>
      <Dialog open={asking} onOpenChange={setAsking}><DialogContent className="max-h-[90dvh] overflow-y-auto"><DialogTitle>Dúvida sobre a proposta</DialogTitle><p>{payload.editor.project.title}</p>{query.data.questionReceived ? <p>Sua dúvida já foi registrada para o responsável. Para continuar a conversa, use o WhatsApp.</p> : <form className="space-y-3" onSubmit={e=>{e.preventDefault();question.mutate({token,text:questionText});}}><label className="block space-y-2"><span>O que você gostaria de esclarecer?</span><Textarea required minLength={2} maxLength={2000} rows={5} value={questionText} onChange={e=>setQuestionText(e.target.value)}/></label><p className="text-sm text-muted-foreground">O responsável poderá consultar sua mensagem no orçamento e responder pelo contato cadastrado.</p><button type="submit" className="proposal-action-primary" disabled={question.isPending || questionText.trim().length<2}>{question.isPending?"Registrando…":"Enviar dúvida"}</button></form>}{questionLink&&<a className="proposal-action-secondary" href={questionLink} target="_blank" rel="noreferrer">Continuar no WhatsApp</a>}</DialogContent></Dialog>
