...
{stock.filter(m=>m.kind==="ink"||m.kind==="diluent").map(m=>{const row=ings.find(x=>x.materialId===m.id),value=row?.drops??0;return <div className="mix-ingredient" key={m.id}>
  <div className="mix-ingredient-head"><span className="mix-ingredient-name">{m.name}</span><div className="mix-number-wrap"><input aria-label={"Gotas de "+m.name} type="number" inputMode="numeric" pattern="[0-9]*" min="1" max={cupDropCapacity} placeholder="0" value={row?String(row.drops):""} onFocus={e=>{const el=e.currentTarget.closest(".mix-ingredient");setTimeout(()=>el?.scrollIntoView({block:"center",behavior:"smooth"}),180)}} onChange={e=>{const raw=e.target.value;if(raw==="")return setIng(m.id,0);const n=Math.min(cupDropCapacity,Math.max(0,Math.floor(Number(raw)||0)));setIng(m.id,n)}}/><span>gt</span></div></div>
