...
function inferKind(name:string,category?:string|null):Kind{const t=((category||"")+" "+name).toLowerCase();if(/diluent|diluente|mixing|solucao de mistura/.test(t))return"diluent";if(/tinta|pigment|ink|dynamic|electric ink/.test(t))return"ink";if(/cartucho|agulha|needle|round liner|round shader|magnum|\brl\b|\brs\b/.test(t))return"cartridge";if(/vaselina|butter|pomada|karit|slip/.test(t))return"ointment";return"protection"}
...
function useMat(m:Material){if(m.kind==="cartridge")return void quickConsume(m,"1.000",m.name);if(m.kind==="protection")return void quickConsume(m,"1.000",m.name);if(m.kind==="ointment")return void quickConsume(m,"10.000",m.name+" +10 g");setInk(m);setSheet("ink")}
...
{sheet==="recipe"&&<section className="cockpit-sheet"><button className="close" onClick={()=>setSheet(null)}><X size={17}/></button><h3>Receita {sample?"· "+sample.code:""}</h3><p>Será gravada junto da sessão e das baixas de cada pigmento.</p><div className="sheet-grid">{(["P","M","G","GG"] as Cup[]).map(c=><button key={c} className={"sheet-option "+(cup===c?"selected":"")} onClick={()=>setCup(c)}><b>Batoque {c}</b><small>{CUP[c]} ml · ~{Math.round(CUP[c]/DROP)} gotas</small></button>)}</div>
