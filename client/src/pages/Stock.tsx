...
export default function Stock() {
  ...
  const [movementMaterialId, setMovementMaterialId] = useState<number | null>(
    null,
  );
  ...
  const [historyMaterialId, setHistoryMaterialId] = useState<number | null>(
    null,
  );
  ...
  const { data: materials = [], isLoading } = trpc.stock.listMaterials.useQuery(
    { activeOnly: true },
  );
  const { data: lowStock = [] } = trpc.stock.getLowStock.useQuery();
  const { data: reorderSuggestions = [] } =
    trpc.stock.getReorderSuggestions.useQuery();
  const { data: expiryAlerts = [] } = trpc.stock.getExpiryAlerts.useQuery({
    days: 90,
  });
  ...
  const handleSubmit = () => {
    ...
    const minimumConversion =
      form.minStockUnit === "package"
        ? parseFloat(form.unitsPerPackage) || 1
        : 1;
    const normalizedMinimum =
      (parseFloat(form.minStock) || 0) * minimumConversion;
    const payload = {
      name: form.name,
  ...
  const handleMovementSubmit = () => {
    ...
    const material = materials.find(
      (candidate) => candidate.id === movementMaterialId,
    );
    const conversionFactor =
      movForm.type === "ajuste" || movForm.inputUnit === "base"
        ? 1
    ...
    addMovement.mutate({
      ...
      inputUnit:
        movForm.inputUnit === "package"
          ? material?.purchaseUnit || "embalagem"
          : material?.baseUnit || material?.unit || "un",
      conversionFactor,
      lotNumber: movForm.lotNumber.trim() || undefined,
      expiresAt: movForm.expiresAt
        ? `${movForm.expiresAt} 00:00:00`
        : undefined,
      alertAt: movForm.alertAt ? `${movForm.alertAt} 00:00:00` : undefined,
      source:
  ...
  const addKitItem = () => {
    setKitForm((form) => ({
      ...form,
      items: [...form.items, { materialId: "", quantity: "1", unit: "un" }],
    }));
  };
  ...
  const statusBadge = (status: string) => {
    ...
    if (status === "low")
      return <Badge className="bg-yellow-600 text-white">Estoque baixo</Badge>;
    return (
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-muted-foreground text-xs sm:text-sm mt-1">
              Gerencie os insumos e materiais do estúdio
            </p>
          </div>
          <Button
            onClick={() => {
        ...
        <div className="flex flex-wrap gap-2 border-b pb-4">
          <Button
            variant={stockView === "inventory" ? "default" : "outline"}
            size="sm"
            onClick={() => setStockView("inventory")}
            className={
              stockView === "inventory"
          ...
          <Button
            variant={stockView === "catalog" ? "default" : "outline"}
            size="sm"
            onClick={() => setStockView("catalog")}
            className={
              stockView === "catalog" ? "bg-orange-500 hover:bg-orange-600" : ""
            }
          >
            Catálogo técnico
          </Button>
        ...
        {stockView === "catalog" ? (
          ...
          <>
                  ...
                  <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:flex-wrap">
                    {reorderSuggestions.map((m) => (
                      <Badge
                        key={m.id}
                        variant="outline"
                        className="h-auto max-w-full justify-start whitespace-normal break-words border-yellow-600 py-1.5 text-left text-xs leading-relaxed text-yellow-300"
                      >
                        {m.name} — pedir {m.suggestedPackages} {m.purchaseUnit}{" "}
                        ({m.suggestedBaseUnits} {m.baseUnit})
                      </Badge>
                    ))}
                  </div>
                </CardContent>
            ...
            {expiryAlerts.length > 0 && (
              <Card className="min-w-0 overflow-hidden border-red-600/40 bg-red-950/15">
                <CardHeader className="pb-2">
                  <CardTitle className="flex items-start gap-2 text-sm text-red-400">
                    <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
                    <span className="min-w-0">
                      Materiais para repor antes do vencimento
                    </span>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:flex-wrap">
                    {expiryAlerts.map((lot) => (
                      <Badge
                        key={lot.id}
                        variant="outline"
                        className={`h-auto max-w-full justify-start whitespace-normal break-words py-1.5 text-left text-xs leading-relaxed ${Number(lot.daysRemaining) < 0 ? "border-red-600 text-red-300" : "border-amber-600 text-amber-300"}`}
                      >
                        {lot.materialName} · lote {lot.lotNumber} —{" "}
                        {Number(lot.daysRemaining) < 0
                          ? `vencido há ${Math.abs(Number(lot.daysRemaining))} dias`
                          : `vence em ${lot.daysRemaining} dias`}
                        {lot.alertAt &&
                          ` · aviso definido para ${new Date(lot.alertAt).toLocaleDateString("pt-BR")}`}
                      </Badge>
                    ))}
                  </div>
                </CardContent>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="hidden md:table-cell text-right text-xs sm:text-sm">
                        Mínimo
                      </TableHead>
                      <TableHead className="hidden lg:table-cell text-right text-xs sm:text-sm">
                        Preço
                      </TableHead>
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell className="text-right font-mono text-xs sm:text-sm">
                              ...
                              >
                                {parseFloat(String(mat.currentStock))}{" "}
                                {mat.baseUnit || mat.unit}
                              </span>
                              {Number(mat.unitsPerPackage) > 1 && (
                                <span className="block text-[10px] text-muted-foreground">
                                  ≈{" "}
                                  {(
                                    Number(mat.currentStock) /
                                    Number(mat.unitsPerPackage)
                            ...
                            <TableCell className="hidden md:table-cell text-right font-mono text-xs text-muted-foreground">
                              {parseFloat(String(mat.minStock))} {mat.unit}
                            </TableCell>
                            ...
                            <TableCell>
                              <div className="flex gap-0.5 justify-end">
                                ...
                                <Button
                                  ...
                                  title="Histórico"
                                  onClick={() => {
                                    setHistoryMaterialId(mat.id);
                                    setHistoryMaterialName(mat.name);
                                    setShowHistory(true);
                                  }}
                                >
                  ...
                  <p className="text-2xl font-bold text-red-400">
                    {
                      materials.filter(
                        (m) =>
                          parseFloat(String(m.currentStock)) <= 0 &&
                          parseFloat(String(m.minStock)) > 0,
                      ).length
                    }
                  </p>
                </CardContent>
              ...
              <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Conjuntos de materiais aplicáveis em um clique na POD
                    Session.
                  </p>
                </div>
                <Button
              ...
              <CardContent>
                {kits.length === 0 ? (
                  <p className="text-sm text-muted-foreground">
                    Nenhum kit cadastrado. Crie um kit para acelerar o
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    {["un", "cx", "pct", "frasco", "galão", "rolo"].map((u) => (
                      <SelectItem key={u} value={u}>
                        {u}
                      </SelectItem>
                    ))}
                ...
                <div className="grid grid-cols-[minmax(0,1fr)_110px] gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setForm((f) => ({
                        ...f,
                        minStockUnit: v as "base" | "package",
                      }))
                    }
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      <SelectItem value="base">
                        {form.unit === "un" ? "Unidade" : form.unit}
                      </SelectItem>
                      <SelectItem value="package">
                        {form.purchaseUnit === "cx"
                          ? "Caixa"
                          : form.purchaseUnit}
                      </SelectItem>
                    </SelectContent>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, targetStock: e.target.value }))
                  }
                />
              </div>
              <div>
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          setShowKitForm(open);
          if (!open) setKitForm(emptyKitForm());
        }}
      >
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  value={kitForm.name}
                  onChange={(e) =>
                    setKitForm((form) => ({ ...form, name: e.target.value }))
                  }
                  placeholder="Ex: Kit Preto e Cinza"
                />
              </div>
              <div>
                <Label>Categoria</Label>
              ...
              <div className="sm:col-span-2">
                ...
                <Textarea
                  ...
                  rows={2}
                  placeholder="Quando este kit deve ser usado?"
                />
              </div>
            </div>
            <div className="space-y-2">
              ...
              {kitForm.items.map((item, index) => (
                <div
                  key={index}
                  className="grid grid-cols-[minmax(0,1fr)_90px_70px_auto] gap-2 items-end"
                >
                  <div className="min-w-0">
                    <Label className="text-xs">Material</Label>
                    <Select
                      value={item.materialId || "unselected"}
                      onValueChange={(value) => {
                        const material = materials.find(
                          (candidate) => String(candidate.id) === value,
                        );
                        updateKitItem(index, {
                          materialId: value === "unselected" ? "" : value,
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      value={item.quantity}
                      onChange={(e) =>
                        updateKitItem(index, { quantity: e.target.value })
                      }
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      value={item.unit}
                      onChange={(e) =>
                        updateKitItem(index, { unit: e.target.value })
                      }
                    />
                  </div>
                  <Button
                    ...
                    size="icon"
                    className="h-9 w-9"
                    disabled={kitForm.items.length === 1}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowKitForm(false)}>
              Cancelar
            </Button>
            <Button onClick={handleKitSubmit} disabled={createKit.isPending}>
          ...
          <div className="space-y-4">
            ...
            {movForm.type !== "ajuste" && (
              <div>
                ...
                <Select
                  value={movForm.inputUnit}
                  onValueChange={(v) =>
                    setMovForm((f) => ({
                      ...f,
                      inputUnit: v as "base" | "package",
                    }))
                  }
                >
                  <SelectTrigger>
            ...
            {(() => {
              ...
              return (
                <div className="rounded-lg border border-orange-500/25 bg-orange-500/[0.06] p-3 text-sm">
                  <span className="font-medium">Movimento calculado:</span>{" "}
            ...
            {movForm.type === "entrada" && (
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                <div>
                  ...
                  <Input
                    value={movForm.lotNumber}
                    onChange={(e) =>
                      setMovForm((f) => ({ ...f, lotNumber: e.target.value }))
                    }
                ...
                <div>
                  ...
                  <Input
                    type="date"
                    value={movForm.expiresAt}
                    onChange={(e) =>
                      setMovForm((f) => ({ ...f, expiresAt: e.target.value }))
                    }
                  />
                </div>
                <div>
                  <Label>Data do aviso (opcional)</Label>
                  <Input
                    type="date"
                    max={movForm.expiresAt || undefined}
                    value={movForm.alertAt}
                    onChange={(e) =>
                      setMovForm((f) => ({ ...f, alertAt: e.target.value }))
                    }
                  />
                </div>
                <p className="text-xs text-muted-foreground sm:col-span-3">
                  Se o aviso for informado, o sistema lembrará da reposição a
                  partir dessa data. Sem aviso personalizado, mantém-se a
                  antecedência padrão.
                </p>
              </div>
            )}
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setMovForm((f) => ({ ...f, reason: e.target.value }))
                }
                  ...
                  movements.map((m) => (
                    <TableRow key={m.id}>
                      ...
                      <TableCell className="text-right font-mono">
                        {parseFloat(String(m.quantity))}
                      </TableCell>
                      <TableCell className="text-right font-mono text-muted-foreground">
                        {parseFloat(String(m.previousStock))}
                      </TableCell>
                      <TableCell className="text-right font-mono font-bold">
                        {parseFloat(String(m.newStock))}
                      </TableCell>
                      <TableCell className="text-xs text-muted-foreground">
                        {m.reason || "—"}
                      </TableCell>
                  ...
                  <TableBody>
                    {lots.map((lot) => (
                      <TableRow key={lot.id}>
                        ...
                        <TableCell className="text-xs">
                          {lot.alertAt
                            ? new Date(lot.alertAt).toLocaleDateString("pt-BR")
                            : "Padrão do sistema"}
                        </TableCell>
