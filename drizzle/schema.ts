...
export const anamneseRequests = mysqlTable(
  "anamnese_requests",
  {
    ...
    statusRequest: mysqlEnum([
      ...
      .default("pendente")
      .notNull(),
    source: mysqlEnum(["public_link", "legacy_csv"])
      .default("public_link")
      .notNull(),
    importBatchId: int(),
    originalArtistName: varchar({ length: 255 }),
    procedureDate: datetime({ mode: "string" }),
    procedureDateStatus: mysqlEnum(["inferred", "confirmed"])
      .default("inferred")
      .notNull(),
  ...
  (table) => [index("anamnese_requests_token_unique").on(table.token)],
);
...
export const anamnesisRiskHistory = mysqlTable(
  "anamnesis_risk_history",
  {
    id: int().autoincrement().primaryKey().notNull(),
  ...
  (table) => [
    index("risk_history_studio_created_idx").on(
      table.studioId,
      table.createdAt,
    ),
    index("risk_history_client_created_idx").on(
      table.clientId,
      table.createdAt,
    ),
  ],
...
export const appointments = mysqlTable("appointments", {
  ...
  confirmationAttention: mysqlEnum([
    "none",
    "pending",
    "accepted",
    "resolved",
  ...
  signalStatus: mysqlEnum(["aguardando_sinal", "sinal_confirmado"]).default(
    "aguardando_sinal",
  ),
...
export const clients = mysqlTable("clients", {
  ...
  loyaltyLevel: mysqlEnum(["Bronze", "Prata", "Ouro"])
    .default("Bronze")
    .notNull(),
  createdAt: timestamp({ mode: "string" })
    .default(sql`CURRENT_TIMESTAMP`)
...
export const studios = mysqlTable(
  ...
  (table) => [index("studios_masterKey_unique").on(table.masterKey)],
);
...
export const users = mysqlTable(
  "users",
  {
    ...
    role: mysqlEnum(["superadmin", "admin", "collaborator"])
...
export const catalogBrands = mysqlTable(
  "catalog_brands",
  {
    id: int().autoincrement().primaryKey().notNull(),
    name: varchar({ length: 255 }).notNull(),
...
export const catalogProductLines = mysqlTable(
  ...
  (table) => [
    uniqueIndex("catalog_product_lines_brand_name_unique").on(
      table.brandId,
      table.name,
    ),
...
export const catalogVariants = mysqlTable(
  "catalog_variants",
  {
    id: int().autoincrement().primaryKey().notNull(),
    lineId: int().notNull(),
...
export const supplierCatalogOfferings = mysqlTable(
  "supplier_catalog_offerings",
  {
    id: int().autoincrement().primaryKey().notNull(),
    ...
    variantId: int(),
    sourceUrl: varchar({ length: 1000 }),
    evidenceStatus: mysqlEnum(["item", "marca", "pendente"])
      .default("pendente")
      .notNull(),
...
export const stockMovements = mysqlTable("stock_movements", {
  ...
  alertAt: datetime({ mode: "string" }),
  source: mysqlEnum(["manual", "procedimento", "compra", "ajuste"])
    .default("manual")
    .notNull(),
  notes: text(),
  createdBy: int(),
...
export const materialLots = mysqlTable(
  "material_lots",
  {
    ...
    expiresAt: datetime({ mode: "string" }),
    alertAt: datetime({ mode: "string" }),
    currentQuantity: decimal({ precision: 12, scale: 3 })
      .default("0")
      .notNull(),
    supplierId: int(),
  ...
  (table) => [
    uniqueIndex("material_lots_material_lot_unique").on(
      table.materialId,
      table.lotNumber,
    ),
...
export const purchaseOrderItems = mysqlTable("purchase_order_items", {
  ...
  notes: text(),
  receivedQuantity: decimal({ precision: 12, scale: 3 }),
  receivedBaseQuantity: decimal({ precision: 12, scale: 3 }),
  receivedLotNumber: varchar({ length: 100 }),
  receivedExpiresAt: datetime({ mode: "string" }),
  receivedAlertAt: datetime({ mode: "string" }),
  qualityStatus: mysqlEnum([
    "nao_verificada",
    "aprovado",
    "ressalva",
    "recusado",
...
export const postSaleFollowups = mysqlTable(
  "post_sale_followups",
  {
    id: int().autoincrement().primaryKey().notNull(),
    appointmentId: int(),
    ...
    status: mysqlEnum([
      ...
      .default("scheduled")
      .notNull(),
    deliveryMode: mysqlEnum(["manual", "automatic"])
      .default("manual")
      .notNull(),
    message: text(),
    source: mysqlEnum(["appointment", "legacy_anamnesis"])
      .default("appointment")
      .notNull(),
  ...
  (table) => [
    uniqueIndex("post_sale_followups_appointment_stage_unique").on(
      table.appointmentId,
      table.stage,
    ),
    uniqueIndex("post_sale_followups_submission_stage_unique").on(
      table.anamnesisSubmissionId,
      table.stage,
    ),
    index("post_sale_followups_studio_date_idx").on(
      table.studioId,
      table.scheduledAt,
    ),
...
export const legacyImportBatches = mysqlTable(
  "legacy_import_batches",
  {
    ...
    status: mysqlEnum(["processing", "completed", "failed"])
      .default("processing")
      .notNull(),
  ...
  (table) => [
    uniqueIndex("legacy_import_batch_file_unique").on(
      table.studioId,
      table.targetArtistId,
      table.fileHash,
    ),
...
export const legacyImportRows = mysqlTable(
  "legacy_import_rows",
  {
    id: int().autoincrement().primaryKey().notNull(),
    batchId: int().notNull(),
  ...
  (table) => [
    uniqueIndex("legacy_import_row_fingerprint_unique").on(
      table.studioId,
      table.fingerprint,
    ),
    index("legacy_import_row_batch_idx").on(
      table.batchId,
...
export const waitlistEntries = mysqlTable(
  "waitlist_entries",
  {
    ...
    status: mysqlEnum(["active", "contacted", "booked", "paused", "cancelled"])
      .default("active")
      .notNull(),
    notes: text(),
    createdAt: timestamp({ mode: "string" })
  ...
  (table) => [
    index("waitlist_entries_studio_status_idx").on(
      table.studioId,
      table.status,
    ),
...
export const passwordResetTokens = mysqlTable(
  ...
  (table) => [uniqueIndex("idx_password_reset_token").on(table.token)],
);
...
export const procedureConsumables = mysqlTable("procedure_consumables", {
  ...
  materialLotId: int(), // lote efetivamente utilizado, quando informado
  lotNumber: varchar({ length: 100 }),
  expiresAt: datetime({ mode: "string" }),
  category: mysqlEnum([
    "ink",
...
export type InsertAnamnesisRiskHistory =
  typeof anamnesisRiskHistory.$inferInsert;
...
export const whatsappIntegrations = mysqlTable("whatsapp_integrations", {
  ...
  status: mysqlEnum(["ativo", "inativo", "erro", "aguardando"])
    .default("aguardando")
    .notNull(),
...
export const messageQueue = mysqlTable("message_queue", {
  ...
  status: mysqlEnum(["pendente", "enviada", "erro", "cancelada", "respondida"])
    .default("pendente")
    .notNull(),
