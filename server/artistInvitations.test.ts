...
describe('permissões reais dos novos artistas',()=>{
 ...
 it('nega lembretes de agendamento pertencente a outro artista',async()=>{mock.permissions.mockResolvedValue(true);database([[{studioId:10,artistId:77}]]);await expect(assertInvitedArtistAccess(user,'appointments.reminders.list','query',{appointmentId:99})).rejects.toThrow('Seu acesso');});
 it('impede colaborador de reatribuir o próprio agendamento a outro artista',async()=>{mock.permissions.mockResolvedValue(true);database([[{studioId:10,artistId:7}]]);await expect(assertInvitedArtistAccess(user,'appointments.update','mutation',{id:99,data:{artistId:77}})).rejects.toThrow('Seu acesso');});
