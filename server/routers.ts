...
import { publicProcedure, protectedProcedure, artistProcedure, router } from "./_core/trpc";
...
export const appRouter = router({
  ...
  kits: router({
    ...
    get: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        const kit = await db.getProcedureKitById(input.id, ctx.user.studioId ?? 1);
        if (!kit) throw new TRPCError({ code: "NOT_FOUND", message: "Kit não encontrado" });
        return kit;
      }),
    ...
    applyToProcedure: protectedProcedure
      .input(z.object({ kitId: z.number().int().positive(), procedureId: z.number().int().positive() }))
      .mutation(async ({ ctx, input }) => {
        try {
          ...
          const message = error instanceof Error ? error.message : "Não foi possível aplicar o kit";
        ...
        try {
          ...
          if (!studioId) {
            if (ctx.user.role === 'superadmin') {
              ...
              if (!firstStudio) {
                throw new TRPCError({ code: "PRECONDITION_FAILED", message: "Nenhum estúdio cadastrado no sistema. Crie um estúdio primeiro." });
              }
              studioId = firstStudio.id;
            } else {
              throw new TRPCError({ code: "FORBIDDEN", message: "Usuário não vinculado a um estúdio. Acesse Configurações para selecionar seu estúdio." });
            }
          }
    ...
    getById: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const rows = await d
          .select()
          .from(appointments)
          .where(and(eq(appointments.id, input.id), eq(appointments.studioId, studioId)))
          .limit(1);
        return rows[0] ?? null;
      }),
  ...
  reports: router({
    ...
    paymentMethodBreakdown: protectedProcedure
      ...
      .query(async ({ input }) => {
        return await db.getPaymentMethodBreakdown(input.startDate, input.endDate);
      }),
  ...
  catalog: router({
    ...
    search: protectedProcedure
      .input(z.object({
        ...
        formats: z.array(z.string().trim().min(1).max(100)).max(12).optional(),
  ...
  collaboratorReports: router({
    ...
    summary: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        return db.getCollaboratorsSummary(studioId, input.period, input.referenceDate);
      }),
  }),
