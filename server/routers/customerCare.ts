...
import {and,eq,desc,isNull,ne,notInArray} from 'drizzle-orm';
import {QUOTE_AUTOMATIC_TAGS,isAutomaticQuoteTag} from '../../shared/quoteHistory';
...
export const customerCareRouter=router({
 ...
 linkFeedback:tenantProcedure.input(z.object({id:z.number(),targetId:z.number()})).mutation(async({ctx,input})=>{manager(ctx);const db=await database();const event=(await db.select().from(careEvents).where(and(eq(careEvents.id,input.id),eq(careEvents.studioId,ctx.studioId))).limit(1))[0];const target=(await db.select().from(careEvents).where(and(eq(careEvents.id,input.targetId),eq(careEvents.studioId,ctx.studioId))).limit(1))[0];if(!event?.feedback||event.artistId||!target?.artistId||target.clientId!==event.clientId)throw new TRPCError({code:'BAD_REQUEST',message:'Escolha um atendimento deste cliente.'});await db.update(careEvents).set({artistId:target.artistId,appointmentId:target.appointmentId,readAt:null}).where(and(eq(careEvents.id,event.id),eq(careEvents.studioId,ctx.studioId)));return{ok:true};}),
 ...
 saveTags:tenantProcedure.input(z.object({clientId:z.number(),labels:z.array(z.string().trim().min(1).max(60)).max(32)})).mutation(async({ctx,input})=>{await requireClient(ctx,input.clientId);const db=await database();await db.transaction(async tx=>{await tx.delete(careTags).where(and(eq(careTags.studioId,ctx.studioId),eq(careTags.clientId,input.clientId),notInArray(careTags.label,QUOTE_AUTOMATIC_TAGS)));const labels=normalizeCareTags(input.labels).filter(label=>!isAutomaticQuoteTag(label));if(labels.length>30)throw new TRPCError({code:'BAD_REQUEST',message:'Use até 30 etiquetas manuais.'});if(labels.length)await tx.insert(careTags).values(labels.map(label=>({studioId:ctx.studioId,clientId:input.clientId,label})));});return{ok:true};}),
