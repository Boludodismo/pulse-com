...
async function assertProcedureLinks(
  db: Awaited<ReturnType<typeof requireDb>>,
  input: { studioId: number; clientId: number; appointmentId?: number | null; artistId?: number | null },
) {
  ...
  if (input.appointmentId) {
    const [appointment] = await db.select({ clientId: appointments.clientId, artistId: appointments.artistId }).from(appointments).where(and(
    ...
    if (appointment.clientId !== input.clientId) throw new TRPCError({ code: "BAD_REQUEST", message: "O agendamento selecionado pertence a outro cliente." });
    if (input.artistId && appointment.artistId && input.artistId !== appointment.artistId) {
      throw new TRPCError({ code: "BAD_REQUEST", message: "O artista informado não corresponde ao agendamento." });
    }
  }
  ...
  if (input.artistId) {
    ...
    if (!artist) throw new TRPCError({ code: "NOT_FOUND", message: "Artista não encontrado nesta empresa." });
  }
}
...
export const proceduresRouter = router({
  ...
  update: tenantProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await assertProcedureLinks(db, {
        ...
        appointmentId: input.appointmentId !== undefined ? input.appointmentId : existing.appointmentId,
        artistId: input.artistId !== undefined ? input.artistId : existing.artistId,
      });
  ...
  updateConsumable: tenantProcedure
    .input(z.object({
      id: z.number().int().positive(),
      procedureId: z.number().int().positive(),
      quantity: z.number().min(0),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [existing] = await db
        ...
        .from(procedureConsumables)
        .where(and(
          eq(procedureConsumables.id, input.id),
          eq(procedureConsumables.procedureId, input.procedureId),
        ))
        .limit(1);
  ...
  // ── Remover insumo ───────────────────────────────────────────────────────
  removeConsumable: tenantProcedure
    .input(z.object({
      id: z.number().int().positive(),
      procedureId: z.number().int().positive(),
    }))
  ...
  consumableReport: tenantProcedure
    .input(z.object({
  ...
  consumableSummary: tenantProcedure
    .query(async ({ ctx }) => {
      const studioId = ctx.studioId;
      const db = await requireDb();
      const now = new Date();
  ...
  listLinkedAppointmentIds: tenantProcedure
    .query(async ({ ctx }) => {
      const studioId = ctx.studioId;
      const db = await requireDb();
      const rows = await db
...
export const proceduresInternals = { assertProcedureOwner };
